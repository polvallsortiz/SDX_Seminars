...
request(Ref, Msg, Nodes, 0) ->
    ...
    lists:foreach(fun(Node) ->
                    [{FromName, From, Msgtext, SenderN, Nre, Niter}] = Msg,
                    Iternew = Niter + 1,
                  NewMsg = {FromName, From,Msgtext, SenderN, Nre, Iternew},
                  Node ! {request, Self, Ref, {FromName, From,Msgtext, SenderN, Nre, Iternew}}
                  end, 
                  Nodes);
request(Ref, Msg, Nodes, Jitter) ->
    Self = self(),
    lists:foreach(fun(Node) ->
                    ...
                    timer:send_after(T, Node, {request, Self, Ref, {FromName, From,Msgtext, SenderN, Nre, Iternew}} )
                  end,
                  Nodes).
...
deliver(Master, Messages) ->
    lists:foreach(fun(Msg)->
                    ...
                    Iternew = Niter + 1,
                    NewMsg = {FromName, From,Msgtext, SenderN, Nre, Iternew},
                      Master ! {deliver, {FromName, From,Msgtext, SenderN, Nre, Iternew}}
                  end, 
                  Messages).
