...
worker(Name, Id, Cast, NextSnd, NextRcv, Sleep, Tref) ->
    receive
        {deliver, {FromName, From, Msg, SenderN, Nre, Niter}} ->
            io:format("~s RECV(~4w) ; From: ~s(~4w) ; Subject: ~s~n; Number of submessages ~s(~4w)",
                      [Name, NextRcv, FromName, SenderN, Msg, Niter]),
            if
                From == Id ->
